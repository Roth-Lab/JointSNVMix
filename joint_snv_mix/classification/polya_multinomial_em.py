...
from jsm_models.em.extended_multinomial.polya import JointExtendedMultinomialModel
...
#    chr_list = reader.get_chr_list()
    ...
    for chr_name in sorted( chr_list ):
        nrows = reader.get_chr_size( chr_name )
        
        start = 0
        end = min( start + buffer_size, nrows )
        ...
        while end < nrows:
            ...
            counts = reader.get_counts( chr_name, indices )
            
            data = MultinomialData( counts )
            
            responsibilities = model.classify( data, parameters )
            
            jcnt_rows = reader.get_rows( chr_name, indices )
...
def parse_parameters_file( parameters_file_name ):
    ...
    parameters = {}
    parameters['pi'] = np.zeros( ( nclass ** 2, ) )
    parameters['alpha'] = np.zeros( ( 2, nclass, 4 ) )
    ...
    for i, genotype in enumerate( constants.extended_multinomial_genotypes ):
        ...
        for j, nuc in enumerate( constants.nucleotides ):
            ...
            parameters['alpha'][0, i, j] = parser.getfloat( 'alpha', normal_genotype_nuc )
            parameters['alpha'][1, i, j] = parser.getfloat( 'alpha', tumour_genotype_nuc )
