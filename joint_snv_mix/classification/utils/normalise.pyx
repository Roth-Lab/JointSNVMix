...
def log_space_normalise_rows(np.ndarray[np.double_t, ndim=2] mat):
    '''
    Normalises the rows of a matrix in logspace to prevent underflow.
    
    Input is log_X where X is the matrix to be normalised.
    '''
