...
def parse_priors_file( priors_file_name, n ):
    ...
    priors['location'] = np.zeros( ( 2, 9, 2 ) )
    priors['precision'] = np.zeros( ( 2, 9, 2 ) )
    ...
    for i, genotype in enumerate( constants.joint_genotypes ):
        joint_genotype = "_".join( genotype )
        
        normal_genotype = "_".join( ( 'normal', joint_genotype ) )
        tumour_genotype = "_".join( ( 'tumour', joint_genotype ) )
...
def parse_parameters_file( parameters_file_name ):
    ...
    parameters = {}
    parameters['alpha'] = np.zeros( ( 2, 9 ) )
    parameters['beta'] = np.zeros( ( 2, 9 ) )
