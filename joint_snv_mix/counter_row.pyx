...
cdef class RowFactory(object):
    ...
    cdef JointBinaryCounterRow get_row(
            self,
            char * ref,
            int pos,
    ...
    cdef char * _get_var_base(self, char * ref_base, PileupColumn normal_column, PileupColumn tumour_column):
        ...
        counts[3].base = "T"
        for i in range(4):
            normal_counts = normal_column.get_nucleotide_count(
                counts[i].base,
                self._min_base_qual,
                self._min_map_qual
            )
            tumour_counts = tumour_column.get_nucleotide_count(
                counts[i].base,
                self._min_base_qual,
                self._min_map_qual
            )
...
cdef class QualityDataFactory(DataFactory):
    ...
    cdef _get_aligment_probabilities(
            self,
            char * ref_base,
            char * var_base,
            double * q,
        ...
        Since we are only given probabilities for observed base, we assume the remaining probability is evenly shared
        among the 3 other bases.
